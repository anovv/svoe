...
from data_catalog.pipelines.index_cryptotick.dag import split_l2_inc_df_and_pad_with_snapshot
...
class TestIndexCryptotickPipeline(unittest.TestCase):
    ...
    def test_split_l2_inc_df_and_pad_with_snapshot(self):
        ...
        splits_with_snapshot = split_l2_inc_df_and_pad_with_snapshot(big_df, 10*1024)
        splits_to_concat = []
        for i in range(len(splits_with_snapshot)):
            split = splits_with_snapshot[i]
            assert starts_with_snapshot(split)
            assert get_snapshot_depth(split) == 5000 * 2 # bids + asks
            if i > 0:
                split_no_snap = remove_snap(split)
                splits_to_concat.append(split_no_snap)
