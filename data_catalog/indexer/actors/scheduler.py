...
class Scheduler:
    ...
    async def stats_loop(self):
        while self.is_running:
            metadata = []
            for workflow_id, workflow_status in workflow.list_all():
    ...
    async def scheduler_loop(self):
        while self.is_running:
            ...
            is_last = 'is_last' in input_batch[0]
            ...
            # construct DAG
            filtered_items = workflow.continuation(filter_existing.options(**workflow.options(task_id=filter_task_id)).bind(self.db_actor, input_batch))
            ...
            self.workflow_task_ids[workflow_id]['download_task_ids'] = download_task_id
    ...
    async def _wait_for_workflows_to_finish(self):
        to_wait = workflow.list_all({WorkflowStatus.RUNNING, WorkflowStatus.PENDING})
        while len(to_wait) != 0:
            await asyncio.sleep(0.1)
            to_wait = workflow.list_all({WorkflowStatus.RUNNING, WorkflowStatus.PENDING})
