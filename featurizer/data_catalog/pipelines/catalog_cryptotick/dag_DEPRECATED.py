...
class CatalogCryptotickDag_DEPRECATED(Dag):
    ...
    def get(self, dag_id: str, input_batch: InputItemBatch, stats: Stats, db_actor: DbActor):
        ...
        for i in range(len(filtered_items)):
            ...
            download_task = load_df.options(**workflow.options(task_id=download_task_id), num_cpus=0.001, resources={'worker_size_large': 1, 'instance_spot': 1}).bind(
            ...
            # splits = workflow.continuation(split_l2_inc_df.options(**workflow.options(task_id=split_task_id), num_cpus=0.9, resources={'worker_size_large': 1, 'instance_spot': 1}).bind(
            ...
            for j in range(len(splits)):
                ...
                catalog_task = catalog_df.options(**workflow.options(task_id=catalog_task_id), num_cpus=0.9, resources={'worker_size_large': 1, 'instance_spot': 1}).bind(
                ...
                store_task = store_df.options(**workflow.options(task_id=store_task_id), num_cpus=0.01, resources={'worker_size_large': 1, 'instance_spot': 1}).bind(
        ...
        scheduled_events_reported = gather_and_wait.options(resources={'worker_size_large': 1, 'instance_spot': 1}).bind([
            send_events_to_stats.bind(stats, download_task_ids, ray_task_name(load_df), EventType.SCHEDULED, stats_extras),
        ])

        gathered_store_tasks = gather_and_wait.options(resources={'worker_size_large': 1, 'instance_spot': 1}).bind(store_tasks)
        gathered_catalog_tasks = gather_and_wait.options(resources={'worker_size_large': 1, 'instance_spot': 1}).bind(catalog_tasks)
        # TODO verify all is stored successfully here?
        gathered_catalog_tasks = chain_no_ret.options(resources={'worker_size_large': 1, 'instance_spot': 1}).bind(gathered_catalog_tasks, gathered_store_tasks, scheduled_events_reported)

        write_catalog_task_id = f'{dag_id}_{ray_task_name(write_batch)}'
        dag = write_batch.options(**workflow.options(task_id=write_catalog_task_id), num_cpus=0.01, resources={'worker_size_large': 1, 'instance_spot': 1}).bind(
