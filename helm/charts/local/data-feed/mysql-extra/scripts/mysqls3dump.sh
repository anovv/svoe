...
aws s3api head-object --bucket ${AWS_S3_BUCKET} --key ${s3filename_prev} || prev_not_exist=true
if [ $prev_not_exist ]; then
  echo "[$(date -Iseconds)] Prev version does not exist"
else
  aws s3api head-object --bucket ${AWS_S3_BUCKET} --key ${s3filename_latest} || latest_not_exist=true
  if [ $latest_not_exist ]; then
    echo "[$(date -Iseconds)] Latest version does not exist, but prev exists, shouldn't happen..."
  else
    aws s3 rm "s3://${AWS_S3_BUCKET}/${s3filename_prev}"
    echo "[$(date -Iseconds)] Removed current prev"
    aws s3 mv "s3://${AWS_S3_BUCKET}/${s3filename_latest}" "s3://${AWS_S3_BUCKET}/${s3filename_prev}"
    echo "[$(date -Iseconds)] Moved latest version to prev"
    aws s3 rm "s3://${AWS_S3_BUCKET}/${s3filename_prev}"
    echo "[$(date -Iseconds)] Removed current latest"
  fi
fi
aws s3 cp "${filename}.gz" "s3://${AWS_S3_BUCKET}/${s3filename_latest}"
